...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `ssafit_final`.`video` (
  ...
  `video_viewCnt` INT NULL DEFAULT '0',
  `video_zzimCnt` INT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `ssafit_final`.`videocomment` (
  ...
  CONSTRAINT `videocomment_ibfk_1`
    FOREIGN KEY (`videocomment_video_id`)
    REFERENCES `ssafit_final`.`video` (`video_id`)
    ON DELETE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
